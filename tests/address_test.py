...
class TestAddressFunctions(unittest.TestCase):
    ...
    def test_p2pkh_addr_format_is_valid_mainnet(self):
        ...
        res = l.w_generate_priv_pub_key_pair()
        self.assertTrue(l.w_verify_p2pkh_address(res[1]))
    ...
    #     res = l.w_generate_hd_master_pub_key_pair()
    #     self.assertTrue(l.w_verify_master_priv_pub_keypair(res[0], res[1], chain_code=0))

    # def test_master_keypair_is_valid_testnet(self):
    #     """Test function verifies a valid hd keypair for testnet"""
    #     res = l.w_generate_hd_master_pub_key_pair(chain_code=1)
    #     self.assertTrue(l.w_verify_master_priv_pub_keypair(res[0], res[1], chain_code=1))

    def test_derived_hd_pubkey_gen_mainnet(self):
        """Test function returns derived key on mainnet"""
        keypair = l.w_generate_hd_master_pub_key_pair()
        res = l.w_generate_derived_hd_pub_key(keypair[0])
        self.assertIsNotNone(res)

    def test_derived_hd_pubkey_gen_testnet(self):
        """Test function returns derived key on mainnet"""
        keypair = l.w_generate_hd_master_pub_key_pair(chain_code=1)
        res = l.w_generate_derived_hd_pub_key(keypair[0])
        self.assertIsNotNone(res)

    def test_derived_keypair_is_valid_mainnet(self):
        """Test function verifies a valid derived hd keypair for mainnet"""
        keypair = l.w_generate_hd_master_pub_key_pair(chain_code=0)
        derived_pubkey = l.w_generate_derived_hd_pub_key(keypair[0])
        self.assertTrue(l.w_verify_master_priv_pub_keypair(keypair[0], derived_pubkey, chain_code=0))

    def test_derived_keypair_is_valid_testnet(self):
        """Test function verifies a valid derived hd keypair for testnet"""
        keypair = l.w_generate_hd_master_pub_key_pair(chain_code=1)
        derived_pubkey = l.w_generate_derived_hd_pub_key(keypair[0])
        self.assertTrue(l.w_verify_master_priv_pub_keypair(keypair[0], derived_pubkey, chain_code=1))

    def test_derived_p2pkh_addr_format_is_valid_mainnet(self):
        """Test function returns valid derived pubkey for mainnet"""
        keypair = l.w_generate_hd_master_pub_key_pair(chain_code=0)
        derived_pubkey = l.w_generate_derived_hd_pub_key(keypair[0])
        self.assertTrue(l.w_verify_p2pkh_address(derived_pubkey))
